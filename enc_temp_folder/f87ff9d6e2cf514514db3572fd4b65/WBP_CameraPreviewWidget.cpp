...
void UWBP_CameraPreviewWidget::SelectPreview(int Index)
{
    ...
    // Get the player controller's pawn (Director Pawn)
    APlayerController* PC = GetWorld()->GetFirstPlayerController();
    if (!PC) return;

    APawn* Pawn = PC->GetPawn();
    ACameraDirectorPawn* Director = Cast<ACameraDirectorPawn>(Pawn);
    if (!Director) return;

    // Teleport the DirectorPawn to the preview location
    FTransform NewTransform = Preview->GetActorTransform();
    Director->SetActorTransform(NewTransform, false, nullptr, ETeleportType::TeleportPhysics);

    // Also snap the spring arm + camera to correct relative orientation
